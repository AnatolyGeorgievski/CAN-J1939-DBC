...
Ресурс типа `type:"pwm"` позволяет экспортировать линии ШИМ контроллера. У настроек есть период обновления `max-age`, если данные не обновляются, то настройка сбрасывается к значению по умолчанию. 
...
+ n  - имя объекта (name), имя устройства должно быть уникальное для поиска по имени;
